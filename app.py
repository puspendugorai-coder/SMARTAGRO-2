...
def get_weather():
    ...
    try:
        ...
        # Generate mock NASA MODIS NDVI data based on location
        seed_val = int(abs(float(lat) * 100) + abs(float(lon) * 100))
        random.seed(seed_val)
        ndvi_val = round(random.uniform(0.3, 0.85), 2)
        if ndvi_val < 0.4:
            veg_status = "Sparse Vegetation / Bare Soil"
        elif ndvi_val < 0.6:
            veg_status = "Moderate Vegetation"
        else:
            veg_status = "Healthy Crop"
...
def get_monthly_alerts():
    ...
    for i in range(30):
        ...
        # Mocking data to display UI properly
