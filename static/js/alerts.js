...
function showWeeklyBuffering() {
    ...
    if (list && (!dailyAlertsData || dailyAlertsData.length === 0)) {
        list.innerHTML = `
...
async function checkUpcomingRisks() {
    ...
    try {
        ...
        if (dangerDays.length > 0 && document.getElementById('tabWeekly') && document.getElementById('tabWeekly').classList.contains('active')) {
            showAppNotification({
...
function switchPeriodTab(period) {
    ...
    if (period === 'normal') {
        ...
        if (typeof dailyAlertsData !== 'undefined' && dailyAlertsData && dailyAlertsData.length > 0) {
            selectDay(0);
        }
