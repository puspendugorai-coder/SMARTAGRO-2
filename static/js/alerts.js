...
function reRenderAlerts() {
    ...
    // Re-render day tabs so day names get translated
    if (dailyAlertsData) {
        renderDayTabs(dailyAlertsData);
        // Re-highlight the currently selected day tab
        document.querySelectorAll('.day-tab').forEach((tab, i) => {
            tab.classList.toggle('active', i === selectedDayIndex);
        });
    }
...
document.addEventListener('langChanged', async (e) => {
    ...
    // 1. Fetch the static term translation dict (UI labels, pest names, etc.)
    await loadAlertsTranslations(newLang);

    // 2. If we have live weather, re-fetch today's alerts in the new language
    //    so AI-generated alert card titles/messages/actions get translated.
    if (currentWeather) {
        try {
            ...
            console.warn('[Alerts] Re-fetch on lang change failed:', err);
        }

        // 3. Re-fetch day-wise forecast alerts if they were already loaded
        if (dailyAlertsData && currentForecast) {
            try {
                ...
                renderDayTabs(dailyAlertsData);
                // Show the same day index as before
                if (dailyAlertsData[selectedDayIndex]) {
                    allAlerts = dailyAlertsData[selectedDayIndex].alerts || [];
                    renderAlertsList(allAlerts);
                    updateSummaryCounts(allAlerts);
                }
            } catch (err) {
                console.warn('[Alerts] Re-fetch forecast on lang change failed:', err);
            }
        }
    }
});
...
async function loadAlertsData(lat, lon) {
    ...
    try {
        ...
        // 2. Fetch alerts + translations in parallel so both are ready at the
        //    same moment — translation happens INSIDE the loading buffer, not after.
        ...
        // Translations fetch runs at the same time as alerts (both in parallel)
        let translationsPromise = Promise.resolve(null);
        if (lang !== 'en') {
            showAlertsOverlay(lang); // show "Translating…" during the buffer
            translationsPromise = fetch('/api/translate-alerts', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ lang })
        ...
        // Wait for BOTH before touching the DOM
        const [alertsData, txData] = await Promise.all([alertsPromise, translationsPromise]);

        allAlerts = alertsData.alerts || [];

        // Apply translations before any render so the first paint is translated
        if (txData && txData.translations) {
            _alertsTx = txData.translations;
            _alertsTxCache[lang] = _alertsTx;
        }
        ...
        // Render all sections — already translated on first paint
        renderAlertsList(allAlerts);
        renderPestCalendar(currentWeather);
        renderPesticideSafety(currentWeather);
        renderHarmfulSafeCrops(currentWeather);
        ...
        // Apply [data-translate] static strings (filter tabs, labels, etc.)
        if (typeof applyTranslations === 'function') applyTranslations();

        // Lift overlay only after everything is rendered
        hideAlertsOverlay();
...
/* ══════════════════════════════════════════════
   ...
   crops. Triggered manually via the button so today's
   alerts always load fast on page open.
══════════════════════════════════════════════ */
async function checkUpcomingRisks() {
    if (!currentForecast || currentForecast.length === 0) {
        showToast('Forecast data not available yet.', 'warning');
        return;
    }
    ...
    try {
        ...
        const res = await fetch('/api/alerts-forecast', {
            ...
            body: JSON.stringify({
                ...
                lat: currentWeather.lat,
                lon: currentWeather.lon,
                lang: (localStorage.getItem('agrosmart_lang') || 'en').toLowerCase().trim()
            })
        });
        const data = await res.json();
        dailyAlertsData = data.daily || [];
        ...
        const dangerDays = (data.summary && data.summary.danger_days) || [];
        if (dangerDays.length > 0) {
            showAppNotification({
                id: 'calamityAlertPopup',
                icon: 'fa-triangle-exclamation',
                title: 'Upcoming Weather Risk Detected',
                message: `${dangerDays.length} of the next ${dailyAlertsData.length} days show critical conditions for your crops.`,
                listItems: dangerDays.map(d => `<strong>${getDayName(d.date, false)}</strong>: ${d.titles.join(', ')}`),
                primaryLabel: 'Got it',
            });
        } else {
            showToast('✅ No critical weather risks in the next 6 days.', 'success');
        }
        ...
        showToast('Could not load the 6-day forecast risks.', 'error');
        if (btn) {
            btn.disabled = false;
...
function selectDay(index) {
    ...
    updateSummaryCounts(allAlerts);
    renderAlertsList(allAlerts);
    // Re-apply current filter after day change (preserve active selection)
...
function filterAlerts(filter) {
    ...
    // Update tab styles — match by the onclick filter value, not by display text
    document.querySelectorAll('.alert-tab').forEach(tab => {
        tab.classList.remove('active');
        // Extract the filter argument from onclick="filterAlerts('...')"
        const onclickAttr = tab.getAttribute('onclick') || '';
        const match = onclickAttr.match(/filterAlerts\(['"](.+?)['"]\)/);
        const tabFilter = match ? match[1] : '';
        if (tabFilter === filter) {
