...
/* ── On dashboard load: reuse a location already granted
   earlier this session (e.g. from another page) so the
   user is never asked twice in the same app session ──── */
...
async function loadCropRecommendations(current) {
    try {
        ...
        // dt() is already populated by loadWeatherAndCrops' parallel translation fetch,
        // so crops/calendar/pesticides render translated on the very first paint.
        renderCrops(data);
