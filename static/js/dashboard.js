...
async function loadWeatherAndCrops(lat, lon) {
    ...
    if (lang !== 'en' && !window._dashTranslateInProgress) {
        // Use cached translations if available (instant), otherwise show overlay
        const cached = Object.keys(window._dashTrans || {}).length > 0
            ? Promise.resolve({ translations: window._dashTrans })
            : null;
...
function renderVegetationHealth(vegData) {
    ...
    // Get today's date for 'Last observed' to mock real-time satellite pass
    ...
    container.innerHTML = `
        ...
        <div class="veg-title-wrapper">
            ...
            <span class="veg-title-sub">(NASA MODIS NDVI)</span>
        </div>
        <div class="veg-score-main">${vegData.ndvi}</div>
        <div class="veg-status-main">${dt(vegData.status) || vegData.status}</div>
        
        <div class="veg-progress-bar">
            <div class="veg-progress-fill" style="width: ${vegData.ndvi * 100}%"></div>
        </div>
        
        <div class="veg-last-observed">Last observed: ${today}</div>
        <div class="veg-desc-main">Reflects whatever is currently growing on this land (or bare soil) — not a prediction of future crop health.</div>
    `;
}